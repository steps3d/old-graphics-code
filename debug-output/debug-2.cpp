...
void	dumpLogMessages ()
{
	...
	GLuint	id;
    int		len, loggedMessages;
	
	glGetIntegerv ( GL_DEBUG_LOGGED_MESSAGES_ARB,  &loggedMessages );
	
	for ( int i = 0; i < loggedMessages; i++ )
	{
		...
		glGetDebugMessageLogARB ( 1, len, &source, &type, &id, &severity, NULL, msg );
		
		printf ( "-----------------------------------------------------------------------------------\n" );
		printf ( "Error:\n\tSource: %s\n\tType: %s\n\tId: %d\n\tSeverity: %s\n\tMessage: %s\n",
				stringForSource ( source ), stringForType ( type ), id, stringForSeverity ( severity ), msg );
...
int main ( int argc, char * argv [] )
{
	...
	if ( !GLEW_ARB_debug_output )
	{
		...
		return 2;
					...
					// query properties of debug log
	GLint	maxMessages, maxMessageLength;
	
	glGetIntegerv ( GL_MAX_DEBUG_LOGGED_MESSAGES_ARB, &maxMessages );
	glGetIntegerv ( GL_MAX_DEBUG_MESSAGE_LENGTH_ARB, &maxMessageLength );
	
	printf ( "Max logged messages: %d\n", maxMessages );
